    ...
    <q-card class="q-dialog-plugin">
      <q-card-section>
        <div class="std-dialog-title q-pa-md">
          {{ existingRecordId ? "Editing a Repayment Received Record" : "Adding a Repayment Received Record" }}
        </div>
        <q-form class="q-gutter-md q-pa-md" ref="recordForm">
          ...
          <q-input type="number" filled v-model="recordAmount" label="Received Amount" lazy-rules :rules="validators.balance">
...
const recordType = RecordType.REPAYMENT_RECEIVED;
...
onMounted(async () => {
  if (props.existingRecordId) {
    ...
    initialDoc = res;
    if (!initialDoc.repaymentReceived) {
      // TODO show error message
      isLoading.value = false;
      return;
    }
    recordAmount.value = asAmount(initialDoc.repaymentReceived.amount);
    recordCurrencyId.value = initialDoc.repaymentReceived.currencyId;
    recordPartyId.value = initialDoc.repaymentReceived.partyId;
    recordWalletId.value = initialDoc.repaymentReceived.walletId!;
    ...
    recordCurrencyId.value = props.suggestedCurrencyId ?? null;
    recordPartyId.value = props.suggestedPartyId ?? null;
    recordAmount.value = props.suggestedAmount ?? 0;
  }
});
