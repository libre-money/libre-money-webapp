  ...
  <q-dialog ref="dialogRef" @hide="onDialogHide" no-backdrop-dismiss :maximized="$q.screen.lt.sm">
    <q-card class="q-dialog-plugin column full-height">
      <q-card-section class="no-shrink">
        <div class="std-dialog-title text-warning text-weight-bold">⚠️ Write Off Payable</div>
      </q-card-section>
      <q-separator />
      <q-card-section class="col scroll" style="min-height: 0">
        <q-form class="q-gutter-md q-pa-md1" ref="writeOffForm">
          <q-banner class="bg-warning text-dark">
            ...
            You are about to write off an unpaid payable. This will close the unpaid amount without making a payment. This typically represents waived fees, forgiven amounts, or settlement agreements.
          </q-banner>
          ...
          <div class="row q-gutter-sm">
            <q-btn flat label="Write Off Half" @click="amountToWriteOff = Math.floor(payableItem.amountUnpaid / 2)" />
            <q-btn flat label="Write Off All" @click="amountToWriteOff = payableItem.amountUnpaid" />
          </div>
          ...
          <q-banner class="bg-orange-2 text-dark">
            ...
            This will reduce your liability and create an accounting entry. This action cannot be undone.
          </q-banner>
        </q-form>
      </q-card-section>
      <q-separator />
...
const amountToWriteOff = ref<number>(props.payableItem.amountUnpaid);
const reason = ref<string>("waived");
const tagIdList = ref<string[]>([]);
const notes = ref<string | null>(null);
const transactionEpoch = ref<number>(Date.now());

const reasonOptions = [
  { label: "Waived by vendor", value: "waived" },
...
async function okClicked() {
  ...
  const confirmed = await new Promise((resolve) => {
    $q.dialog({
      ...
      message: `You are about to write off ${printAmount(amountToWriteOff.value)} for ${props.payableItem.partyName}. This action cannot be undone.`,
      cancel: true,
      persistent: true,
    })
      .onOk(() => resolve(true))
  ...
  // Create a PAYABLE_PAYMENT record with amount = amountToWriteOff but walletId = null
  // This indicates it's a write-off, not an actual payment
  const record: Record = {
  ...
  $q.notify({
    type: "positive",
    message: "Payable written off successfully",
  });
