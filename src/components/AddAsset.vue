    ...
    <q-card class="q-dialog-plugin column full-height">
      ...
      <q-card-section class="col scroll" style="min-height: 0">
        <q-form class="q-gutter-md" ref="assetForm">
          <q-input hide-bottom-space standout="bg-primary text-white" v-model="assetName" label="Name of the Asset"
            lazy-rules :rules="validators.name" />
          <q-select standout="bg-primary text-white" v-model="assetType" :options="assetTypeList" label="Type"
            emit-value map-options />
          <q-select standout="bg-primary text-white" v-model="assetLiquidity" :options="assetLiquidityList"
            label="Liquidity" emit-value map-options />
          ...
          <q-toggle v-model="shouldShowAdvancedOptions" color="green" label="Show advanced options" right-label
            v-if="existingAssetId" />
          <q-input type="number" hide-bottom-space standout="bg-primary text-white" v-model="assetInitialBalance"
      ...
      <q-card-section class="no-shrink">
        <div class="flex">
          ...
          <q-btn rounded color="primary" :label="existingAssetId ? 'Update' : 'Add'" @click="okClicked" />
        </div>
      </q-card-section>
    </q-card>
  </q-dialog>
