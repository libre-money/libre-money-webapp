...
import { Collection, rowsPerPageOptions, walletTypeList } from "./../constants/constants";
...
async function addWalletClicked() {
  $q.dialog({ component: AddWallet }).onOk(() => {
    loadData();
  });
}
...
async function editClicked(wallet: Wallet) {
  $q.dialog({ component: AddWallet, componentProps: { existingWalletId: wallet._id } }).onOk(() => {
    loadData();
  });
}
