...
function applyOrdering(docList: IncomeSource[], sortBy: string, descending: boolean) {
  if (sortBy === "name") {
    docList.sort((a, b) => {
...
async function addIncomeSourceClicked() {
  $q.dialog({ component: AddIncomeSource }).onOk(() => {
    loadData();
  });
}
...
async function editClicked(incomeSource: IncomeSource) {
  ...
  $q.dialog({ component: AddIncomeSource, componentProps: { existingIncomeSourceId: incomeSource._id } }).onOk(() => {
    loadData();
  });
}
